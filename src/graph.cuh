...
template <class EdgeType> void CSR<EdgeType>::SetFrontierToRatio(float ratio) {
  ...
  for (uint32 i = 0; i < *numPartitions; i++) {
    ...
    double partRatio = 0;
    uint64 safeEnd = (end > start + 14000) ? end - 14000 : start;
    for (uint32 vertexId = start; vertexId < safeEnd; vertexId++) {
...
template <class EdgeType> void CSR<EdgeType>::InitFrontierNValues() {
  ...
  case BFS:
    ...
    break;
  case SSSP:
    // INF sentinel: max value of EdgeType minus 1, to avoid overflow when
    // adding edge weights.  The previous sentinel (*numVertices + 1) was too
    // small for graphs whose shortest-path distances exceed the vertex count
    // (common with large-weight edges), causing updates to be silently
    // dropped by the `newValue < d_values[neighborId]` check in the kernels.
    // Source distance must be 0 (not 1) so every reachable vertex ends up
    // with its true shortest-path weight rather than (1 + weight).
    for (uint32 i = 0; i < *numVertices; i++) {
      h_frontier[i] = 0;
      h_values[i] = std::numeric_limits<EdgeType>::max() - 1;
    }
    h_frontier[srcVertex] = 1;
    h_values[srcVertex] = 0;
    break;
  case CC:
    for (uint32 i = 0; i < *numVertices; i++) {
      h_frontier[i] = 1;
...
void CSR<EdgeType>::ReadInputFile(const std::string &filePath,
  ...
  // Allocate edges (and weights for SSSP) on the default NUMA node.  Leave the
  // memory unpinned for now so that OS file cache / dirty pages can still be
  // reclaimed while we stream the graph in.  Pinning the full edges+weights
  // buffer (~68 GB for GAP-kron/GAP-urand on the 64-bit weighted path) before
  // reading made the kernel OOM-kill the process on smaller hosts, since
  // pinned pages are unswappable and compete for RAM with the file cache
  // generated by the subsequent infile.read().  Pinning is deferred until
  // after the file read below.
  ...
  // Pointer to the weight destination: h_weights2 is the single authoritative
  // host weight array used by demand/filter/static kernels.  We write weights
  // directly into it, eliminating the former h_weights intermediate copy.
  EdgeType *wDst = (algorithm == SSSP) ? h_weights2[GPUAffinityMap[0]] : nullptr;

  if (isBwcsr64) {
    // bwcsr64: interleaved {uint64 dest, uint64 weight} per edge.
    // Stream in fixed-size chunks to avoid allocating a numEdges-sized temp buf.
    struct OutEdgeWeighted64 {
      uint64 end;
      uint64 w8;
    };

    constexpr uint64 CHUNK = 1ULL << 20; // 1M edges per chunk
    std::vector<OutEdgeWeighted64> buf(std::min(numEdges, CHUNK));

    for (uint64 pos = 0; pos < numEdges; pos += CHUNK) {
      uint64 cnt = std::min(CHUNK, numEdges - pos);
      infile.read((char *)buf.data(), cnt * sizeof(OutEdgeWeighted64));

      for (uint64 i = 0; i < cnt; i++)
        h_edges2[GPUAffinityMap[0]][pos + i] =
            static_cast<EdgeType>(buf[i].end);

      if (wDst) {
        for (uint64 i = 0; i < cnt; i++)
          wDst[pos + i] = static_cast<EdgeType>(buf[i].w8);
      }
    }
  } else if (isBcsr64) {
    ...
    // bwcsr: interleaved {uint32 dest, uint32 weight} per edge.
    ...
    constexpr uint64 CHUNK = 1ULL << 20;
    std::vector<OutEdgeWeighted> buf(std::min(numEdges, CHUNK));

    for (uint64 pos = 0; pos < numEdges; pos += CHUNK) {
      uint64 cnt = std::min(CHUNK, numEdges - pos);
      infile.read((char *)buf.data(), cnt * sizeof(OutEdgeWeighted));
  ...
  // Drop the input file's page cache — on 64-bit weighted graphs the file is
  // roughly numEdges * 16 bytes (e.g. ~68 GB for GAP-urand) and the kernel
  // will otherwise retain much of it in buff/cache, which combined with the
  // soon-to-be-pinned edges+weights buffers can push system RAM past its
  // limits.  POSIX_FADV_DONTNEED is a hint; ignore failures.
  {
    int fadv_fd = ::open(filePath.c_str(), O_RDONLY);
    if (fadv_fd >= 0) {
      posix_fadvise(fadv_fd, 0, 0, POSIX_FADV_DONTNEED);
      ::close(fadv_fd);
    }
  }

  // Now that the edges (and weights) are fully resident from the read above,
  // pin them with cudaHostRegisterMapped so the demand/filter kernels can
  // access them via zero-copy.  Wrap in GPUAssert so any residual pinning
  // failure surfaces a clear message instead of a later kernel fault.
  GPUAssert(cudaHostRegister(h_edges2[GPUAffinityMap[0]],
                             numEdges * sizeof(EdgeType),
                             cudaHostRegisterMapped));
  if (algorithm == SSSP) {
    GPUAssert(cudaHostRegister(h_weights2[GPUAffinityMap[0]],
                               numEdges * sizeof(EdgeType),
                               cudaHostRegisterMapped));
  }
  ...
  for (const auto &pair : numaNodeToIndex) {
    ...
    if (allocatedNumaNodes.find(numaNode) == allocatedNumaNodes.end()) {
      ...
      GPUAssert(cudaHostRegister(h_edges2[index],
                                 numEdges * sizeof(*h_edges2[index]),
                                 cudaHostRegisterDefault));
      ...
      if (algorithm == SSSP) {
        ...
        // Register memory with CUDA
        GPUAssert(cudaHostRegister(h_weights2[index],
